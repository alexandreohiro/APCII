...
void exibir_funcao_da_reta(double x1, double y1, double x2, double y2) // Verifica se é função y=f(x), senão informa reta vertical
{
    double a = y1 - y2;
    double b = x2 - x1;
    ...
    if (b != 0)
    { // evita dividir por zero com != 0 (diferente de zero)
        double m = -a / b;
        double n = -c / b;
